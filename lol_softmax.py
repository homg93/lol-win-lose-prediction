...
with tf.Session() as sess:
	...
	#input_data = sess.run(hypothesis, feed_dict = {X:[[49,10,50044,30570,92922,45369,40,38,74319,113575,432,361]]})
	#input_data = sess.run(hypothesis, feed_dict = {X:[[49,10,50044,30570,432,361]]})

	#input_data = sess.run(hypothesis, feed_dict = {X:[[44,19,71849,60635,805,861]]})
	#input_data = sess.run(logits2, feed_dict = {X:[[0.48780488,0.25806452,0.41509999,0.1936706,0.43301179,0.49626866]]})
	...
	#print "\ninput data : ",input_data, sess.run(tf.arg_max(input_data,1))
	# else:
